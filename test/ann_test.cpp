...
int lookUp(								// look up name in table
	const std::string&	arg,					// name to look up
...
void readPts(							// read data/query points from file
	ANNpointArray		&pa,			// point array (returned)
	int					&n,				// number of points
	const std::string&	file_nm,		// file name
...
ANNbool getDirective(
    istream		&in,			// input stream
    std::string		&directive)		// directive storage
{
...
int main(int argc, char** argv)
{
	...
	long		clock1;						// clock time
	std::string	directive;					// input directive
	...
	while (getDirective(cin, directive)) {
		...
		else if (directive == "query_size") {
			cin >> query_size;
		}
		else if (directive == "bucket_size") {
			cin >> bucket_size;
		}
		...
		//	shrink_rule option
		//----------------------------------------------------------------
		else if (directive == "shrink_rule") {
			cin >> arg;							// input split_rule name
			shrink = (ANNshrinkRule) lookUp(arg, shrink_table, N_SHRINK_RULES);
			if ((int) shrink >= N_SHRINK_RULES) {		// not something we recognize
		...
		//	label operation
		//----------------------------------------------------------------
		else if (directive == "output_label") {
			cin >> arg;
			if (stats > SILENT)
				cout << "<" << arg << ">\n";
		}
		...
		//		generated).
		//----------------------------------------------------------------
		else if (directive == "gen_query_pts") {
			if (distr == PLANTED) {				// planted distribution
				if (data_pts == NULL) {
		...
		//----------------------------------------------------------------
		else if (directive == "read_query_pts") {
			cin >> arg;							// input file name
			readPts(
				query_pts,						// point array
